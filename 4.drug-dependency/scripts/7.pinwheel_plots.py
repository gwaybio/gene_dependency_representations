...
dependency_file = pathlib.Path(f"{data_directory}/CRISPRGeneEffect.csv").resolve(strict=True)
...
# Load metadata
metadata_df_dir = pathlib.Path("../1.data-exploration/data/metadata_df.parquet").resolve(strict=True)
...
train_and_test_subbed_dir = pathlib.Path("../1.data-exploration/data/train_and_test_subbed.parquet").resolve(strict=True)
...
reactome_dir = pathlib.Path("../3.run-biobombe/gsea_results/combined_z_matrix_gsea_results.parquet")
...
drug_dir = pathlib.Path("../4.drug-dependency/results/combined_latent_drug_correlations.parquet")
drug_df = pd.read_parquet(drug_dir)

# Load PRISM data
top_dir = "../4.drug-dependency"
data_dir = "data"
